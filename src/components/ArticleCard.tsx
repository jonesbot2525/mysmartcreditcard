    ...
    <article className="group bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-200 flex flex-col">
      ...
      <Link href={`/articles/${article.slug.current}`} className="block relative aspect-[16/9] overflow-hidden bg-gray-100">
        {imageUrl ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-[#1A1A2E] to-[#1a3a6e] flex items-center justify-center">
            <svg className="w-12 h-12 text-[#D4A843]/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <Link href={`/articles/${article.slug.current}`} className="flex-1">
          <h3 className="font-bold text-[#1A1A2E] text-base leading-snug mb-2 group-hover:text-[#D4A843] transition-colors line-clamp-2">
            {article.title}
          </h3>
          {article.excerpt && (
        ...
        <Link
          ...
          className="mt-4 text-sm font-semibold text-[#D4A843] hover:text-[#1A1A2E] transition-colors flex items-center gap-1"
        >
          Read more
