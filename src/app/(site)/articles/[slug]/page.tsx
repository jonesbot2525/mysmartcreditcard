...
export async function generateStaticParams() {
  ...
  } catch { /* Sanity not ready */ }
  // Return fallback slugs so pages pre-render
  return FALLBACK_ARTICLES.map((a) => ({ slug: a.slug.current }));
}
...
export async function generateMetadata({ params }: ArticlePageProps): Promise<Metadata> {
  ...
  const fallback = FALLBACK_ARTICLES.find((a) => a.slug.current === slug);
  return { title: fallback?.title ?? "Article", description: fallback?.excerpt };
}
      ...
      return (
        <figure className="my-8">
          <div className="relative w-full aspect-[16/9] rounded-lg overflow-hidden">
            <Image src={imageUrl} alt={value.alt ?? ""} fill className="object-cover" sizes="(max-width: 768px) 100vw, 700px" />
          </div>
          {value.caption && <figcaption className="text-xs text-gray-400 text-center mt-2">{value.caption}</figcaption>}
        </figure>
      );
    },
  },
...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  // Fall back to static data if Sanity has nothing
  if (!article) {
    article = FALLBACK_ARTICLES.find((a) => a.slug.current === slug) ?? null;
    if (article) {
      related = FALLBACK_ARTICLES.filter(
        (a) => a.category === article!.category && a.slug.current !== slug
      ).slice(0, 3);
    }
  }
      ...
      <article className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="flex-1 min-w-0 max-w-3xl">
            ...
            {heroImageUrl && (
              <div className="relative w-full aspect-[16/9] rounded-xl overflow-hidden mb-8 shadow-md">
                <Image src={heroImageUrl} alt={article.title} fill className="object-cover" priority sizes="(max-width: 768px) 100vw, 700px" />
              </div>
            )}
            ...
            {/* No body — show excerpt as placeholder */}
            ...
            {article.affiliateLink && (
              <div className="mt-10 bg-[#1A1A2E] rounded-xl p-6 sm:p-8 text-white">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
                    <p className="text-[#D4A843] text-xs font-semibold uppercase tracking-wider mb-1">Recommended Card</p>
                    <h3 className="text-xl font-bold mb-1" style={{ fontFamily: "var(--font-playfair, Georgia, serif)" }}>
                      {article.affiliateCardName ?? "Top Pick"}
                    </h3>
                    <p className="text-gray-300 text-sm">Ready to earn more points on your next trip?</p>
                  </div>
                  <AffiliateButton
            ...
            {/* Source credit */}
            <SourceCredit sourceTitle={article.sourceTitle} sourceUrl={article.sourceUrl} />
          </div>
            ...
            <div className="sticky top-24 space-y-6">
              ...
              {related.length > 0 && (
                <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
                  <h3 className="font-bold text-[#1A1A2E] text-sm uppercase tracking-wider mb-4">Related Articles</h3>
                  <ul className="space-y-3">
                    {related.map((rel) => (
                      <li key={rel._id}>
                        <Link href={`/articles/${rel.slug.current}`} className="text-sm text-gray-600 hover:text-[#D4A843] transition-colors leading-snug block">
                          {rel.title}
                        </Link>
                      </li>
                    ))}
        ...
        {related.length > 0 && (
          <section className="mt-16 pt-10 border-t border-gray-100">
            <h2 className="text-2xl font-bold text-[#1A1A2E] mb-6" style={{ fontFamily: "var(--font-playfair, Georgia, serif)" }}>
